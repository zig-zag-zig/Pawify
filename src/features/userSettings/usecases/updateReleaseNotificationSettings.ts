...
const rebuildKnownReleasesForSettings = async (
  ...
  await mapWithConcurrency(artistIds, KNOWN_RELEASE_REBUILD_CONCURRENCY, async (artistId) => {
    ...
    const releaseIds = getNotificationCandidateReleases(releases, undefined, settings)
...
export const createUpdateReleaseNotificationSettingsUseCase = (
  ...
  sourcePushToken?: string,
    ...
    const previousSettings = await dependencies.releaseNotificationSettingsRepository.getSettings(userId);
    const savedSettings = await dependencies.releaseNotificationSettingsRepository.saveSettings(userId, settings);

    try {
      await rebuildKnownReleasesForSettings(dependencies, userId, savedSettings);
    } catch (error) {
      await dependencies.releaseNotificationSettingsRepository.saveSettings(userId, previousSettings);
      throw error;
    }

    await Promise.all([
      dependencies.userSettingsNotifier.notifySettingsChanged(userId, savedSettings, sourcePushToken),
