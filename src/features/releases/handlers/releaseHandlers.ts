...
export type ReleaseWirePresenters = {
    ...
    getArtistReleases: (result: Awaited<ReturnType<ReleaseUseCases['getArtistReleases']>>) => unknown;
    getReleaseGroupReleases: (result: Awaited<ReturnType<ReleaseUseCases['getReleaseGroupReleases']>>) => unknown;
    getRelease: (result: NonNullable<Awaited<ReturnType<ReleaseUseCases['getRelease']>>>) => unknown;
};

/** v1 exact old contract: task ids always present, no immediate maps. */
export const releasePresentersV1: ReleaseWirePresenters = {
    getNewReleases: (result) => ({ releases: result.releases, releaseCoverTaskId: result.releaseCoverTaskId }),
    getArtistReleases: (result) => ({ releaseGroups: result.releaseGroups, releaseGroupCoverTaskId: result.releaseGroupCoverTaskId }),
    getReleaseGroupReleases: (result) => ({ releases: result.releases, releaseCoverTaskId: result.releaseCoverTaskId }),
    getRelease: (result) => ({ release: result.release, lyricsTaskId: result.lyricsTaskId, profileImageTaskId: result.profileImageTaskId }),
};
...
export const createReleaseHandlers = (
    ...
    const getNewReleasesHandler = authenticatedHandler('/getNewReleases', async ({ res, userId }) => {
        res.status(200).send(presenters.getNewReleases(await releaseUseCases.getNewReleases(userId)));
    });

    const removeNewReleasesHandler = authenticatedHandler('/removeNewReleases', async ({ req, res, userId }) => {
    ...
    const getArtistReleasesHandler = authenticatedHandler('/getArtistReleases', async ({ req, res, userId }) => {
        const artistId = requireString(req.body, 'artistId');

        res.status(200).send(presenters.getArtistReleases(await releaseUseCases.getArtistReleases(userId, artistId)));
    });

    const getReleaseGroupReleasesHandler = authenticatedHandler('/getReleaseGroupReleases', async ({ req, res, userId }) => {
        const releaseGroupId = requireString(req.body, 'releaseGroupId');

        res.status(200).send(presenters.getReleaseGroupReleases(await releaseUseCases.getReleaseGroupReleases(userId, releaseGroupId)));
    });
    ...
    const verifyReleaseExistenceHandler = authenticatedHandler('/verifyReleaseExistence', async ({ req, res, userId }) => {
        const releaseId = requireString(req.body, 'releaseId');
