...
export type ArtistWirePresenters = {
    ...
    getArtistDetails: (result: NonNullable<Awaited<ReturnType<ArtistUseCases['getArtistDetails']>>>) => unknown;
...
export const artistPresentersV1: ArtistWirePresenters = {
    getFollowing: (result) => ({ artists: result.artists, profileImageTaskId: result.profileImageTaskId }),
    getArtistDetails: (result) => ({ artist: result.artist, profileImageTaskId: result.profileImageTaskId }),
...
export const createArtistHandlers = (
    ...
    const getArtistDetailsHandler = authenticatedHandler('/getArtistDetails', async ({ req, res, userId }) => {
    ...
    const searchArtistsHandler = authenticatedHandler('/searchArtists', async ({ req, res, userId }) => {
        ...
        res.status(200).send(presenters.searchArtists(await artistUseCases.searchArtists(userId, query, offset, limit)));
    });

    const followArtistHandler = authenticatedHandler('/followArtist', async ({ req, res, userId }) => {
        const artistId = requireString(req.body, 'artistId');
    ...
    const unfollowArtistHandler = authenticatedHandler('/unfollowArtist', async ({ req, res, userId }) => {
        const artistId = requireString(req.body, 'artistId');
        const sourcePushToken = optionalString(req.body, 'sourcePushToken');
    ...
    const unfollowArtistsHandler = authenticatedHandler('/unfollowArtists', async ({ req, res, userId }) => {
