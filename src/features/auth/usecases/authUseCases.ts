...
export const createSendOtpUseCase = ({
    accountGateway,
}: Pick<AuthUseCaseDependencies, 'accountGateway'>) => async (email: string): Promise<void> => {
    try {
...
export const createVerifyOtpUseCase = ({
    accountGateway,
}: Pick<AuthUseCaseDependencies, 'accountGateway'>) => async (email: string, otp: string): Promise<string> => {
    try {
...
export const createRevokeTokenUseCase = ({
    accountGateway,
}: Pick<AuthUseCaseDependencies, 'accountGateway'>) => async (userId: string): Promise<void> => {
    try {
        await accountGateway.revokeToken(userId);
    } catch (error) {
        const message = error instanceof Error ? error.message : 'Could not update the sign-in session. Please try again.';
        throw new BadRequestError(message);
    }
};
...
export const createChangeEmailUseCase = ({
    accountGateway,
}: Pick<AuthUseCaseDependencies, 'accountGateway'>) => async (userId: string, email: string): Promise<void> => {
    try {
        await accountGateway.changeEmail(userId, email);
    } catch (error) {
        const message = error instanceof Error ? error.message : 'Could not change email. Please try again.';
        throw new BadRequestError(message);
    }
};
...
export const createDeleteUserAccountUseCase = ({
    accountGateway,
    userAccountCache,
}: AuthUseCaseDependencies) => async (userId: string): Promise<void> => {
