...
const ARTIST_PROFILE_IMAGE_REQUEST_CONCURRENCY = backgroundTaskWorkerConfig.artistProfileImageRequestConcurrency;
...
export const fetchAndUpsertArtistProfileImages = async (
    ...
    await mapWithConcurrency(uniqueArtistLookups, ARTIST_PROFILE_IMAGE_REQUEST_CONCURRENCY, async (lookup) => {
        const artistStartedAt = Date.now();
        const artistId = lookup.artistId;
        const imageCacheKey = getCacheKey(artistId, 'artistImages');
        const providedArtistName = typeof lookup.artistName === 'string' && lookup.artistName.trim().length > 0
            ? lookup.artistName.trim()
            : undefined;
        const providedDiscogsUrls = Array.isArray(lookup.discogsUrls)
            ? lookup.discogsUrls.filter((url) => typeof url === 'string' && url.trim().length > 0)
            : undefined;
        ...
        try {
            ...
            if (discogsUrlsForLookup === undefined) {
                cachedArtistDetailsForSync = await getCachedData<CachedArtistDetails>(getCacheKey(artistId, 'artistDetails'));
                if (cachedArtistDetailsForSync?.artist) {
                    cachedArtistDetailsHit = true;
                    if (!artistNameForLookup && typeof cachedArtistDetailsForSync.artist.name === 'string') {
                    ...
                    const legacyArtist = cachedArtistDetailsForSync.artist as typeof cachedArtistDetailsForSync.artist & {
                        discogsUrls?: unknown;
                    };
                    discogsUrlsForLookup = [
                        ...getExternalLinkUrlsByService(cachedArtistDetailsForSync.artist.externalLinks, 'discogs'),
            ...
            if (artistNameForLookup && Array.isArray(discogsUrlsForLookup) && discogsUrlsForLookup.length > 0) {
                usedLookupBypass = true;
                lookupBypassMusicBrainzCount += 1;
                const discogsStartedAt = Date.now();
                const discogsResult = await getDiscogsData(artistNameForLookup, discogsUrlsForLookup, signal);
                discogsDurationMs = Date.now() - discogsStartedAt;
                lookupDiscogsResultState = discogsResult.image === undefined
                    ? 'undefined'
            ...
            if (ARTIST_PROFILE_IMAGE_DETAIL_LOGS_ENABLED) {
                ...
                const urlState = resolvedUrl === undefined
                    ? 'undefined'
                    : resolvedUrl === null
                        ? 'null'
                        : 'present';
