...
export const syncArtistDetailsDiscogsUrls = async (
    ...
    const cachedArtistDetails = existingDetails ?? await getCachedData<CachedArtistDetails>(getCacheKey(artistId, 'artistDetails'));
    if (!cachedArtistDetails?.artist) {
        return false;
    }

    const legacyArtist = cachedArtistDetails.artist as typeof cachedArtistDetails.artist & { discogsUrls?: unknown };
    ...
    if (canonicalDiscogsUrls(currentDiscogsUrls) === canonicalDiscogsUrls(normalizedNextDiscogsUrls)) {
        return false;
    }

    const ttlInHours = getArtistMetadataCacheTtlHours(ttl);
    const nonDiscogsLinks = (cachedArtistDetails.artist.externalLinks ?? [])
        .filter(link => link.service !== 'discogs');
