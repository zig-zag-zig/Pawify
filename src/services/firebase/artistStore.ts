...
import {
  readFollowingArtistsMap,
  writeFollowingArtistsMap,
} from './followingStore.js';
import {
...
export const saveArtistAndKnownReleasesToDb = async (
  ...
  const summaryToSave = artistSummary
    ? stripDiscogsUrls(artistSummary)
    : existingFollowing;
...
export const deleteArtistFromDb = async (userId: string, artistId: string): Promise<void> => {
  ...
  const followingArtistsMap = await readFollowingArtistsMap(userId);
  const remainingArtistIds = Object.keys(followingArtistsMap)
    .filter((followingArtistId) => followingArtistId !== artistId);
