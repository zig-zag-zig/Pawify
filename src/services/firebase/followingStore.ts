...
import {
  FollowedArtistsMap,
  FollowingArtistDocument,
  FollowingArtistsMap,
} from './types.js';
import {
  getFollowingMapDocRef,
  buildFollowingCollectionPayload,
} from './refs.js';
import {
  isPlainObject,
  stripDiscogsUrls,
...
const normalizeFollowingArtistsMap = (
  value: unknown,
): FollowingArtistsMap => {
  if (!isPlainObject(value)) {
    return {};
...
const normalizeFollowingCollectionDocument = (
  value: unknown,
): FollowingArtistsMap | null => {
  if (!isPlainObject(value)) {
    return null;
  }
...
export const readFollowingArtistsMap = async (
  userId: string,
