...
const normalizeReleaseIds = (releaseIds: string[]): string[] => (
  Array.from(new Set(releaseIds.flatMap((releaseId) => {
...
export const getKnownReleasesFromDb = async (
  userId: string,
): Promise<ArtistsAndReleasesMap> => {
...
export const removeKnownReleaseFromAllUsers = async (
  releaseId: string,
): Promise<string[]> => {
  assertRtdbKey('releaseId', releaseId);
