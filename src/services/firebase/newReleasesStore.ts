...
const normalizeStoredNewRelease = (value: unknown): StoredNewRelease | null => {
  ...
  const releaseGroupId = typeof value.releaseGroupId === 'string' && value.releaseGroupId.trim().length > 0
...
export const normalizeNewReleasesCollectionDocument = (value: unknown): StoredNewReleasesMap | null => {
  if (!isPlainObject(value)) {
    return null;
  }
...
export const mergeNewReleasesIntoState = async (
  ...
  await getNewReleasesMapDocRef(userId).set(
    buildNewReleasesCollectionPayload(newReleasesMap),
    { merge: true },
  );
};

export const readNewReleasesState = async (
  userId: string,
...
const buildNewReleaseCoverPageEntries = (
  newReleasesMap: StoredNewReleasesMap,
) => {
  const releaseIdsByGroup = new Map<string, { releaseGroupId: string; releaseIds: Set<string> }>();
...
export const getNewReleasesSnapshotFromDb = async (userId: string): Promise<NewReleasesSnapshot> => {
...
export const removeNewReleasesFromDb = async (userId: string, releaseIds: string[]): Promise<void> => {
