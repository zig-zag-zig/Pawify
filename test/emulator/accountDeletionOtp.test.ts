...
describe('account deletion (production deleteUserAccount)', { skip: !EMULATOR }, () => {
    it('recursively deletes Firestore user doc + subcollections, knownReleases, passwordResets', async () => {
        const user = await auth.createUser({ email: `del-${Date.now()}@example.com`, password: 'password123' });
        ...
        await userRef.collection('newReleases').doc('maps').set({ rel1: { id: 'rel1', title: 'T' } });
        ...
        // Auth user gone.
        await assert.rejects(() => auth.getUser(userId), (e: unknown) => (e as { code?: string }).code === 'auth/user-not-found');
        // Top-level doc gone.
        assert.ok(!(await userRef.get()).exists);
        // Subcollections gone (the original origin/main bug).
        assert.equal((await userRef.collection('followingArtists').get()).size, 0, 'followingArtists subcollection deleted');
        assert.equal((await userRef.collection('newReleases').get()).size, 0, 'newReleases subcollection deleted');
        // passwordResets gone.
    ...
    it('is idempotent: calling again on an already-deleted user does not throw', async () => {
        const user = await auth.createUser({ email: `idem-${Date.now()}@example.com`, password: 'password123' });
        const userId = user.uid;
