...
describe('expoPushClient', () => {
    describe('sendExpoPushNotifications', () => {
        it('sends messages via Dapr HTTP and returns tickets', async () => {
            ...
            const { sendExpoPushNotifications } = await import('../../../src/services/notifications/expoPushClient.js');
        ...
        it('handles response body without data wrapper', async () => {
            installFetch(() => new Response(JSON.stringify([{ status: 'ok', id: 'ticket-1' }]), { status: 200 }));
    ...
    describe('getExpoPushReceipts', () => {
        it('fetches receipts by ID via Dapr HTTP', async () => {
            installFetch((url, init) => {
                assert.equal(url, 'http://dapr.test/v1.0/invoke/expo/method/--/api/v2/push/getReceipts');
                const body = JSON.parse(String(init.body)) as Record<string, unknown>;
                assert.deepEqual(body, { ids: ['receipt-1', 'receipt-2'] });
                return new Response(JSON.stringify({
                    data: {
                        'receipt-1': { status: 'ok' },
                        'receipt-2': { status: 'error', message: 'DeviceNotRegistered', details: { error: 'DeviceNotRegistered', expoPushToken: 'token-2' } },
                    },
                }), { status: 200 });
            });

            const { getExpoPushReceipts } = await import('../../../src/services/notifications/expoPushClient.js');
