...
| # | Issue | Fix |
|---|-------|-----|
| N1 | Audit findings: `@hono/node-server`, `hono`, `ip-address`, `js-yaml`, `re2`, `undici`, `fast-uri`, `brace-expansion` — **all transitive via dev-only `firebase-tools`**. All have non-breaking fixes. | `npm audit fix` → verify lockfile diff only bumps those ranges → `npm run verify` + `npm run test:integration` (exercises firebase-tools emulators). |
| N2 | `@opentelemetry/core <2.8.0` via `@google-cloud/pubsub` via `firebase-tools` — only fix offered is **breaking** (`firebase-tools@14.23.0` downgrade). | Bump `firebase-tools` to latest first (`15.24.0 → 15.28.1`) and re-audit. If still flagged (it was, for `>=14.24.0`): it is **dev-only** (emulator runner, never shipped in the runtime image — Dockerfile installs `--omit=dev`), so accept + document in this file, or optionally try `overrides: {"@google-cloud/pubsub": {"@opentelemetry/core": "^2.8.0"}}` and validate emulators. Prefer accept+document if the override fights back. |
| N3 | **Broken `uuid` override** — `overrides: {"uuid": "^11.1.1"}` leaves `universal-analytics` with `uuid@14.0.0 invalid: ^11.1.1 …` → `npm ls` errors out (ELSPROBLEMS). | Try removing the override entirely and checking the resulting tree (modern firebase-tools may dedupe cleanly); if old uuid majors reappear and matter, scope the override to the offending package instead of globally. Regenerate the lockfile (`rm -rf node_modules && npm install`) and require `npm ls` to exit clean. Add a comment in `package.json` stating why any override remains. |
| N4 | Minor bumps within semver: `@sentry/node 10.67→10.70`, `firebase-admin 14.2→14.3`, `@types/express 5.0.3→5.0.6`, `@types/node 22.16→22.20` (stay on 22 — matches `engines.node=22`), `typescript 5.8→5.9`. | `npm update` for these ranges; full `verify` after. Do **not** jump express 4→5 or typescript 7 here. |
| N5 | **Unpinned `tsx` via `npx`** in `docker-compose.dev.yml` (`command: npx tsx watch src/index.ts`) downloads whatever is latest at container start (supply-chain + offline breakage), while `package.json` `dev` uses `ts-node`. | Standardize: either pin `tsx` as a devDependency and use it in both, or change the compose command to `npm run dev` (ts-node). Recommend the compose→`npm run dev` route (fewer deps); keep `ts-node` as the single dev runner. |
| N6 | Housekeeping: `npm pkg get engines` fine (node 22 vs local 22.15 OK). Keep `build` running tests before emit (CI contract), but note in README that plain `npm run build` needs Java + firebase-tools (emulator integration test). | Docs-only. |

**Acceptance for the npm phase:** `npm audit` → 0 or only the documented `@opentelemetry/core` dev-only finding; `npm ls` exits 0; `npm run verify` green; Docker image builds (`npm run build:unchecked` needs no new deps).
